...
flags = tf.flags
logging = tf.logging
# flags定义命令行参数的方法第一个是参数名称，第二个参数是默认值，第三个是参数描述
flags.DEFINE_string("model", "small", "A type of model. Possible options are: small, medium, large.")
flags.DEFINE_string("data_path", None, "Where the training/test data is stored.")
flags.DEFINE_string("save_path", None, "Model output directory.")
flags.DEFINE_bool("use_fp16", False, "Train using 16-bit floats instead of 32bit floats")
flags.DEFINE_integer("num_gpus", 1, "If larger than 1, Grappler AutoParallel optimizer "
...
class PTBModel(object):
  ...
  def __init__(self, is_training, config, input_):
    ...
    with tf.device("/cpu:0"):
      # 获取一个已经存在的变量或者创建一个新的变量embedding
  ...
  def _build_rnn_graph_cudnn(self, inputs, config, is_training):
    ...
    self._rnn_params = tf.get_variable("lstm_params", initializer=tf.random_uniform(
